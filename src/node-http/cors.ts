...
import type { OnRequestHandler } from "./on-request-handler";
...
export default function cors(config: CorsConfig): OnRequestHandler {
  if (!config || !config.origins) {
    throw new Error(
      'cors: "origins" is required and must be explicitly configured.',
    );
  }
